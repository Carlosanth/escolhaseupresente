(function Harvey(){
  ...
  // ✅ NOVO: dados do produto atual pra calcular preço por método de pagamento
  let produtoAtualPrecoOriginalCentavos = 0;
  ...
  window.addEventListener('DOMContentLoaded', () => {
      ...
      .onSnapshot((snapshot) => {
       ...
       try {
        ...
        snapshot.forEach((doc) => {
          ...
          img.src = produto.imagem || IMAGEM_PADRAO_URL;
          img.alt = produto.titulo || 'Produto';
          img.onerror = () => { img.src = IMAGEM_PADRAO_URL; };
          ...
          if (produto.disponivel && !cotasAcabaram) {
            if (temCotas) {
              ...
              // ✅ NOVO: dados pra calcular preço por método de pagamento
              btn.dataset.precoOriginalCentavos = produto.preco_original_centavos || produto.preco_centavos || '0';
              btn.dataset.taxaPercentual        = produto.taxa_percentual || '0';
              btn.dataset.taxaQuemPaga          = produto.taxa_quem_paga || 'dono';
    ...
    // ── Confirmar nome ─────────────────────────────────────────
    // ============================================================
    // 💰 Cálculo de preço por método (validado com o usuário)
    // ============================================================
    const TAXAS_ASAAS = {
      PIX: { percentual: 0, fixoCentavos: 199 }, // R$ 1,99 fixo — CONFIRMAR no painel
      CREDIT_CARD: [
        { min: 1,  max: 1,  percentual: 2.99, fixoCentavos: 49 },
        { min: 2,  max: 6,  percentual: 3.49, fixoCentavos: 49 },
        { min: 7,  max: 12, percentual: 3.99, fixoCentavos: 49 },
      ],
    };

    function resolverValorBruto(netAlvoCentavos, percentual, fixoCentavos) {
      const pct = percentual / 100;
      return Math.ceil((netAlvoCentavos + fixoCentavos) / (1 - pct));
    }

    function calcularOpcoesDePagamento({ precoOriginalCentavos, taxaPercentualPlataforma, taxaQuemPaga }) {
      const comissaoCentavos = Math.round(precoOriginalCentavos * taxaPercentualPlataforma / 100);
      const convidadoPagaComissao = taxaQuemPaga === "convidado";
      const netAlvoCentavos = convidadoPagaComissao
        ? precoOriginalCentavos + comissaoCentavos
        : precoOriginalCentavos;

      function calcularOpcao(billingType, label, percentual, fixoCentavos, parcelas) {
        const precoFinalCentavos = resolverValorBruto(netAlvoCentavos, percentual, fixoCentavos);
        return { billingType, parcelas, label, precoFinalCentavos };
      }

      const opcoes = [calcularOpcao("PIX", "Pix", TAXAS_ASAAS.PIX.percentual, TAXAS_ASAAS.PIX.fixoCentavos, null)];
      TAXAS_ASAAS.CREDIT_CARD.forEach(faixa => {
        const label = faixa.min === faixa.max ? "Cartão à vista" : `Cartão em até ${faixa.max}x`;
        opcoes.push(calcularOpcao("CREDIT_CARD", label, faixa.percentual, faixa.fixoCentavos, faixa.max));
      });
      return opcoes;
    }
    ...
    // ============================================================
    function abrirModalMetodoPagamento(opcoes) {
      document.getElementById('modal-metodo-pagamento')?.remove();

      const botoesHTML = opcoes.map((op, i) => {
        const valorFmt = (op.precoFinalCentavos / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
        return `
          <button type="button" class="opcao-escolha opcao-metodo" data-idx="${i}">
            <span class="opcao-escolha-textos">
              <span class="opcao-escolha-titulo">${op.label}</span>
              <span class="opcao-escolha-valor">${valorFmt}</span>
            </span>
          </button>`;
      }).join('');

      const modal = document.createElement('div');
      modal.id = 'modal-metodo-pagamento';
      modal.className = 'modal-container mostrar';
      modal.innerHTML = `
        <div class="modal-conteudo">
          <button type="button" class="modal-fechar-x" id="btn-fechar-metodo-x" aria-label="Fechar">✕</button>
          <h3>Como você quer pagar?</h3>
          <p>O valor muda um pouquinho por método (taxa da operadora).</p>
          ${botoesHTML}
          <div class="modal-botoes" style="margin-top:14px;">
            <button id="btn-voltar-metodo" class="botao">Voltar</button>
          </div>
        </div>`;
      document.body.appendChild(modal);

      const fechar = () => { modal.classList.remove('mostrar'); modal.remove(); };
      modal.addEventListener('click', (e) => { if (e.target === modal) fechar(); });
      document.getElementById('btn-fechar-metodo-x').addEventListener('click', fechar);
      document.getElementById('btn-voltar-metodo').addEventListener('click', fechar);

      modal.querySelectorAll('.opcao-metodo').forEach(btn => {
        btn.addEventListener('click', async () => {
          const op = opcoes[parseInt(btn.dataset.idx)];
          fechar();
          await finalizarCompra(nomeConvidadoAtual, op.billingType, op.parcelas);
        });
      });
    }
    ...
    async function finalizarCompra(nomeConvidado, billingType, parcelas) {
      ...
      try {
        ...
        // ✅ NOVO: método de pagamento escolhido (só no fluxo normal, sem cota).
        // Vai tudo pra finalizarCompra — ela mesma já decide internamente
        // se aplica o preço por método (só quando o dono usa Asaas).
        if (billingType) {
